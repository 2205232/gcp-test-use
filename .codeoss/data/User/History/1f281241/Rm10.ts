...
export class ChatbotComponent  implements AfterViewChecked {
  ...
  onInputChange(event: Event){ //Sneha
    const input = (event.target as HTMLInputElement).value;
    if(this.chatsBackup.length === 0){ //Sneha
      console.log(this.chatsBackup);
    this.chatService.toggleChatSearchVisibility(input.trim().length > 0);
    }
   }
  ...
  sendMessage(){
    ...
    this.headicon = true;
    this.isLoading = true;
    this.chatService.toggleChatSearchVisibility(true);
   ...
   this.addMessage(trimmedSearchText, 'user', 'sent');
    ...
    this.chatService.sendChatRequest(payload).subscribe({
      next: (data) => {
        console.log(data);
        const botResponse = data.results?.answer || 'No response from the bot';// Assign API response
      ...
      error: (error) => {
        console.error('Error from API:', error); // Log any error
      }
    });
